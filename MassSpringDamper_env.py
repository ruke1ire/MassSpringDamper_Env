...
class MassSpringDamperEnv(gym.Env):
    ...
    def __init__(self): #, spring_stiffness, damper_factor, mass, max_force4
        ...
        self.mass = 0.25 #mass
        self.max_force = np.array([1.0]) #max_force
        self.step_length = 0.1 # in seconds
        obs_high = np.array([self.x_treshold,np.finfo(np.float32).max], dtype = np.float32)#,self.x_treshold,np.finfo(np.float32).max])
    ...
    def reset(self,goal_x=-2.0,goal_x_dot=0.0, seed = None, options = None):
